...
class UIObjectType(Enum):  # FIXME: need to consider more phrases
    @classmethod
    def list(cls):
...
def _build_word_sequence(text, content_desc, resource_id):
    ...
    if text or content_desc:
        ...
        if "_" in name:
            remove_none_list = list(filter(None, name.split('_')))
        else:
            remove_none_list = list(filter(None, camel_case_split(name)))
            # remove words such as edit, editing, input

        # remove_none_list = list(filter(None, name.split('_')))
        return [x for x in remove_none_list if
                x.upper() not in UIObjectType.list()]  # remove object type, since when we synthesize action phrase, we would add the object type again
...
def _build_object_type(android_class):
    ...
    """
    # if android_class.startswith('android.widget'): #FIXME: we remove it since it is too strict for me
    ...
    def __init__(self,
        ...
        self.uiobject = UIObject(
            ...
            obj_name=_build_object_name(
                element.get('text'), element.get('content-desc')),  # FIXME: content-desc is list, not a string
...
class ViewHierarchy(object):
    ...
    def _calculate_dom_location(self):
        ...
        for element in self._all_visible_leaves:
            ...
            dom_location_dict[id(element)][DomLocationKey.DEPTH.value] = len(  # id() return memory address
                ancestors)
